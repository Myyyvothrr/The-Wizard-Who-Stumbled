...
.defines(function()
{
	...
	{
		animSheet: new ig.AnimationSheet('media/mage.png', 16, 16),
		...
		ai: function()
		{
			...
			if (this.distanceTo(this._player) > 180)
				return;

			if (this._do_teleport || this.distanceTo(this._player) > 80 && Math.random() > 0.1)
			{
				switch(this._player.direction)
				{
					case DIRECTION.LEFT:
					{
						this._teleport_to.x = this._player.pos.x + 32 + 8 * Math.random();						
						this._teleport_to.y = this._player.pos.y -8  + 16 * Math.random();
						break;
					}
					...
					case DIRECTION.UP:
					{
						this._teleport_to.x = this._player.pos.x -8  + 16 * Math.random();						
						this._teleport_to.y = this._player.pos.y + 32 + 8 * Math.random();
						break;
					}
					case DIRECTION.DOWN:
					{
				...
				{
					ig.game.spawnEntity(EntityImp, this.pos.x + this.size.x * 0.5 , this.pos.y + this.size.x * 0.5, { });
					this._emitter.emit(EntityParticleTeleported, (ig.ua.mobile ? 2 : (8 + 4 * Math.random())), this.pos.x + this.size.x * 0.5, this.pos.y + this.size.y * 0.5, { direction: this.direction });		
				}

				this.pos = this._teleport_to;
				this.direction = this._player.direction;
				this._emitter.emit(EntityParticleTeleported, (ig.ua.mobile ? 4 : (16 + 4 * Math.random())), this.pos.x + this.size.x / 2, this.pos.y + this.size.y / 2, { direction: this.direction });	
